  ...
  function () {
    ...
    _proto._fill = function _fill() {
      ...
      $.each(this._values, function (i, el) {
        ...
        var $option = $("<span class=\"" + _this._options.block + "__option\">" + el + "</span>");
        ...
        if (!$selected.length && i === 0 || el === $selected.text().trim()) {
          if (!placeholder) {
            _this._$value.text(el);
            _this._$value.removeClass(_this._$value.data('class')).removeData('class').addClass(cssClass).data('class', cssClass);
          }
    ...
    _proto._show = function _show(event) {
      ...
      if($('.open').length) $('.open').on('resize scroll', this._dropup);
      else $(window).on('resize scroll', this._dropup);
